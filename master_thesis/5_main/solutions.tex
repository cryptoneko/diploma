...
Введемо деяке невелике $\delta x$,
щоб ймовірність того, что значення потрапляє в проміжок
$\left[ x - \delta x; x + \delta x \right]$
приблизно дорівнювала добутку щільності розподілу на $\delta x$
\begin{equation*}
  \mathbb{P}\left( \xi \in \left[ x - \delta x; x + \delta x \right] \right)
  \approx p\left( x \right) \cdot \delta x.
\end{equation*}
Далі треба розбити простір на гіперкуби
зі стороною $2 \cdot \delta x$ і вважати,
що коли величина потрапляє на територію певного гіперкубу,
то вона дорівнює значенню в його центрі.
Позначимо в рамках даного пункту
\begin{equation*}
  \mathbb{P}_I\left( x \;\middle|\; t \right)
  = \mathbb{P}_I\left( \xi = x \;\middle|\; t \right)
  = \mathbb{P}\left( \xi \in \left[ x - \delta x; x + \delta x \right]
    \;\middle|\; t \right)
  \approx p\left( x \;\middle|\; t \right) \cdot \delta x.
\end{equation*}
